	...
	function imprimirLinea($val,$count,$fill){
		...
		foreach ($this->getDataSource() as $datarow) {			
			...
			if($nro_comprobante == 1){
				$aux=$aux.'Nro Comprobante:'.trim($datarow['nro_comprobante'])."\r\n";
			}else{
				$aux=$aux.'';
			}
			if($nro_tramite == 1){
			...
			if($desc_tipo_relacion_comprobante == 1){
				$aux=$aux.'Tramite:'.strval($datarow['desc_tipo_relacion_comprobante'])."\r\n";
			}else{
				$aux=$aux.'';
			}
					
			if($fecIni == 1){
				$arr = explode('-', $datarow['fecIni']);
				$newDate = $arr[2].'-'.$arr[1].'-'.$arr[0];
			...
			//
			$this->tablealigns=array('C','L','L','R','R');
