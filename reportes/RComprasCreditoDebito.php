...
class RComprasCreditoDebito extends ReportePDF {
	var $datos_titulo;
	var $datos_detalle;
	var $ancho_hoja;
	var $gerencia;
	...
	function imprimirLinea($val,$count,$fill){
		...
		foreach ($this->datos_detalle as $datarow) {						
			...
			$RowArray = array(
				...
				's6' =>$datarow['importe_doc'],
				's7' =>$datarow['importe_doc']*0.13,	
				's8' =>trim($datarow['codigo_control']),
				's9' =>	$datarow['fecha_2'],						
				's10' =>trim($datarow['nro_documento_2']),							
				's11' =>trim($datarow['nro_autorizacion_2']),
				's12' =>trim($datarow['importe_doc_2']),					
			);
			$fill = !$fill;					
