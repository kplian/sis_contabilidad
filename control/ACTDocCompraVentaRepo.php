...
require_once(dirname(__FILE__).'/../reportes/RLcvRepo.php');

class ACTDocCompraVentaRepo extends ACTbase{    
	...
	function recuperarDatosLCV(){    	
		$this->objFunc = $this->create('MODDocCompraVentaRepo');
	...
	function reporteLCV(){
		...
		if($this->objParam->getParametro('tipo')=='compra'){
		   $reporte = new RLcvRepo($this->objParam);  
		}
		else{
