...
CREATE TABLE conta.tconfig_tipo_cuenta(
    id_config_tipo_cuenta SERIAL NOT NULL,
    nro_base int4,
    tipo_cuenta varchar,
    incremento VARCHAR(10) DEFAULT 'debe' NOT NULL,
    eeff VARCHAR(20)[] DEFAULT '{"balance"}' NOT NULL,
    movimiento VARCHAR(20) DEFAULT 'diario' NOT NULL,
    PRIMARY KEY (id_config_tipo_cuenta))
    INHERITS (pxp.tbase);
...
CREATE TABLE conta.tcuenta (
  ...
  valor_incremento VARCHAR(10) DEFAULT 'positivo' NOT NULL,
  eeff VARCHAR(20)[] DEFAULT '{balance}' NOT NULL,
  sw_control_efectivo VARCHAR(6) DEFAULT 'no' NOT NULL,
  id_config_subtipo_cuenta INTEGER,
  tipo_act VARCHAR(15) DEFAULT 'no' NOT NULL,
  act_cbte_apertura VARCHAR(20) DEFAULT 'todos'::character varying NOT NULL,
  cuenta_actualizacion VARCHAR(20),
  ex_auxiliar VARCHAR DEFAULT 'no' NOT NULL,
...
CREATE TABLE conta.tauxiliar (
  ...
  corriente VARCHAR(4) DEFAULT 'no' NOT NULL,
  aplicacion VARCHAR(50),
  estado VARCHAR(10) DEFAULT 'si'::character varying,
...
CREATE TABLE conta.torden_trabajo (
  ...
  tipo VARCHAR(20) DEFAULT 'estadistica' NOT NULL,
  movimiento VARCHAR(5) DEFAULT 'si' NOT NULL,
...
CREATE TABLE conta.tclase_comprobante(
    ...
    tipo_comprobante varchar(300) DEFAULT 'contable',
    codigo VARCHAR(50) NOT NULL,
    momento_comprometido VARCHAR(30) DEFAULT 'opcional' NOT NULL,
    momento_ejecutado VARCHAR(30) DEFAULT 'opcional' NOT NULL,
    momento_pagado VARCHAR(30) DEFAULT 'opcional' NOT NULL,
    tiene_apertura varchar(8) NOT NULL DEFAULT 'no'::character varying, -- esa calse de comprobantes admite cbte de apertura, esto es para saber si va saltar la numeracion o no
	  movimiento varchar(20) NOT NULL DEFAULT 'diario'::character varying, -- diario, ingreso o egreso
    CONSTRAINT tclase_comprobante_codigo_key UNIQUE (codigo),
...
COMMENT ON COLUMN conta.tclase_comprobante.tiene_apertura IS 'esa calse de comprobantes admite cbte de apertura, esto es para saber si va saltar la numeracion o no';
COMMENT ON COLUMN conta.tclase_comprobante.movimiento IS 'diario, ingreso o egreso';
...
CREATE TABLE conta.ttabla_relacion_contable (
  ...
  tabla_id_fk VARCHAR(100),
  recorrido_arbol VARCHAR(20),
  tabla_id_auxiliar VARCHAR(100),
  tabla_codigo_auxiliar VARCHAR(100),
  tabla_codigo_aplicacion VARCHAR(100),
  codigo VARCHAR,
...
CREATE TABLE conta.ttipo_relacion_contable (
  ...
  tiene_auxiliar VARCHAR(20) DEFAULT 'si'::character varying COLLATE pg_catalog."default" NOT NULL ,
  id_tabla_relacion_contable INTEGER,
  partida_tipo VARCHAR(25) DEFAULT 'flujo_presupuestaria',
  partida_rubro VARCHAR(15) DEFAULT 'recurso_gasto'::character varying,
  tiene_aplicacion VARCHAR(10) DEFAULT 'no' NOT NULL,
  tiene_moneda VARCHAR(6) DEFAULT 'no' NOT NULL,
  tiene_tipo_centro VARCHAR(10) DEFAULT 'no' NOT NULL,
  codigo_aplicacion_catalogo VARCHAR(300),
  tiene_apertura VARCHAR(8) DEFAULT 'no' NOT NULL,
  movimiento VARCHAR(20) DEFAULT 'diario' NOT NULL,
...
COMMENT ON COLUMN conta.ttipo_relacion_contable.tiene_apertura
IS 'esa calse de comprobantes admite cbte de apertura, esto es para saber si va saltar la numeracion o no';

COMMENT ON COLUMN conta.ttipo_relacion_contable.movimiento
...
CREATE TABLE conta.trelacion_contable (
  ...
  id_tabla INTEGER,
  defecto VARCHAR(3) DEFAULT 'no' NOT NULL,
  id_moneda INTEGER,
  id_tipo_presupuesto INTEGER,
  codigo_aplicacion VARCHAR(200),
...
CREATE TABLE conta.tplantilla_comprobante (
  ...
  momento_presupuestario VARCHAR(255) DEFAULT 'contable' NOT NULL, 
  ...
  otros_campos VARCHAR(1000) COLLATE pg_catalog."default",
  campo_gestion_relacion VARCHAR(255),
  momento_comprometido VARCHAR(3) DEFAULT 'no' NOT NULL,
  momento_ejecutado VARCHAR(3) DEFAULT 'no' NOT NULL,
  momento_pagado VARCHAR(3) DEFAULT 'no' NOT NULL,
  campo_id_cuenta_bancaria VARCHAR(100),
  campo_id_cuenta_bancaria_mov VARCHAR(100),
  campo_nro_cheque VARCHAR(100),
  campo_nro_tramite VARCHAR(350),
  campo_nro_cuenta_bancaria_trans VARCHAR(350),
  campo_tipo_cambio VARCHAR(350),
  campo_depto_libro TEXT,
  campo_fecha_costo_ini TEXT,
  campo_fecha_costo_fin TEXT,
  funcion_comprobante_editado TEXT,
  funcion_comprobante_prevalidado TEXT,
  funcion_comprobante_validado_eliminado TEXT,
  desc_plantilla VARCHAR DEFAULT '' NOT NULL,
  campo_cbte_relacionado VARCHAR(350),
  codigo_tipo_relacion VARCHAR(350),
  campo_tipo_cambio_2 VARCHAR(350),
  campo_tipo_cambio_3 VARCHAR(350),
  campo_id_config_cambiaria VARCHAR(350),
  CONSTRAINT pk_tplantilla_comprobante__id_plantilla_comprobante PRIMARY KEY (id_plantilla_comprobante),
  CONSTRAINT tplantilla_comprobante_codigo_key1 UNIQUE (codigo)
) INHERITS (pxp.tbase)
WITHOUT OIDS;
...
CREATE TABLE conta.tdetalle_plantilla_comprobante (
  ...
  aplicar_documento VARCHAR(3) DEFAULT 'no', 
  campo_documento TEXT, 
  campo_fecha TEXT, 
  campo_concepto_transaccion TEXT,
  primaria VARCHAR(4) DEFAULT 'si' NOT NULL,
  otros_campos VARCHAR(250),
  campo_partida_ejecucion TEXT,
  mensaje_error VARCHAR(250),
  descripcion varchar(250) DEFAULT 'Transaccion de plantilla' NOT NULL,
  campo_monto_pres VARCHAR(255),
  id_detalle_plantilla_fk INTEGER,
  forma_calculo_monto VARCHAR(50) DEFAULT 'simple' NOT NULL,
  func_act_transaccion VARCHAR(100),
  campo_id_tabla_detalle VARCHAR(100) DEFAULT '',
  rel_dev_pago VARCHAR(3) DEFAULT 'no' NOT NULL,
  campo_trasaccion_dev TEXT,
  nom_fk_tabla_maestro VARCHAR(80),
  campo_id_cuenta_bancaria varchar(100),
  campo_id_cuenta_bancaria_mov varchar(100),
  campo_nro_cheque VARCHAR(100),
  campo_nro_cuenta_bancaria_trans VARCHAR(350),
  campo_porc_monto_excento_var VARCHAR(350),
  campo_nombre_cheque_trans VARCHAR(350),
  prioridad_documento INTEGER DEFAULT 2 NOT NULL,
  campo_orden_trabajo VARCHAR(350),
  campo_forma_pago VARCHAR(350),
  codigo VARCHAR,
  campo_relacion_contable_cc TEXT,
  tipo_relacion_contable_cc VARCHAR(255),
  campo_suborden VARCHAR(350),
  campo_codigo_aplicacion_rc VARCHAR(350),
  incluir_desc_doc VARCHAR(15) DEFAULT 'todos'::character varying NOT NULL COLLATE pg_catalog."default",
  procesar_prioridad_principal VARCHAR(2) DEFAULT 'si'::character varying NOT NULL,
  campo_id_taza_impuesto VARCHAR(350),
  campo_nro_tramite_auxiliar VARCHAR(100),
  insertar_prioridad_principal varchar(2) NOT NULL DEFAULT 'si'::character varying,
...
CREATE TABLE conta.tcomprobante(
	...
	glosa1 varchar(1500),
	glosa2 varchar(400),
	beneficiario varchar(500) COLLATE pg_catalog."default",
...
CREATE TABLE conta.tint_comprobante(
	...
	id_clase_comprobante int4 ,
	id_int_comprobante_fk int4 ,
	id_subsistema int4,
	id_depto int4,
	id_moneda int4,
	id_periodo int4,
	nro_cbte varchar(30),
	momento varchar(30) DEFAULT 'contable' NOT NULL,
	glosa1 varchar(1500),
	glosa2 varchar(400),
	beneficiario varchar(500) COLLATE pg_catalog."default",
	tipo_cambio numeric,
  ...
  momento_comprometido VARCHAR(4) DEFAULT 'no' NOT NULL,
  momento_ejecutado VARCHAR(4) DEFAULT 'no' NOT NULL,
  momento_pagado VARCHAR(4) DEFAULT 'no' NOT NULL,
  id_cuenta_bancaria INTEGER,
  id_cuenta_bancaria_mov INTEGER,
  nro_cheque INTEGER,
  nro_cuenta_bancaria_trans VARCHAR(255),
  id_plantilla_comprobante INTEGER,
  manual VARCHAR(5) DEFAULT 'no' NOT NULL,
  id_int_comprobante_fks INTEGER[],
  id_tipo_relacion_comprobante INTEGER,
  id_depto_libro INTEGER,
  cbte_cierre VARCHAR(20) DEFAULT 'no' NOT NULL,
  cbte_apertura VARCHAR(10) DEFAULT 'no' NOT NULL,
  cbte_aitb VARCHAR(10) DEFAULT 'no' NOT NULL,
  origen VARCHAR(30),
  temporal VARCHAR(3) DEFAULT 'no' NOT NULL,
  id_int_comprobante_origen_central INTEGER,
  vbregional VARCHAR(4) DEFAULT 'no' NOT NULL,
  ...
  sw_tipo_cambio VARCHAR(5) DEFAULT 'no' NOT NULL,
  id_config_cambiaria INTEGER,
  localidad VARCHAR(20) DEFAULT 'nacional' NOT NULL,
  sw_editable VARCHAR(4) DEFAULT 'si' NOT NULL,
  id_int_comprobante_bk integer,
  id_ajuste INTEGER,
  volcado VARCHAR(3) DEFAULT 'no' NOT NULL,
  cbte_reversion VARCHAR(4) DEFAULT 'no' NOT NULL,
  id_proceso_wf INTEGER,
  id_estado_wf INTEGER,
  c31 VARCHAR(200),
  fecha_c31 DATE,
  forma_cambio VARCHAR(20) DEFAULT 'oficial' NOT NULL,
  tipo_cambio_3 NUMERIC,
  id_moneda_act INTEGER,
  glosa_previa VARCHAR DEFAULT '' NOT NULL,
  nro_tramite_aux VARCHAR(70),
  documento_iva VARCHAR(4) DEFAULT 'si',
  id_int_comprobante_migrado INTEGER,
  liquido_pagable decimal(18, 2) default 0,
...
CREATE TABLE conta.ttransaccion(
  ...
  id_transaccion_fk int4 , 
  glosa varchar COLLATE pg_catalog."default",
  id_int_transaccion integer not null,
  id_orden_trabajo INTEGER,
  importe_debe NUMERIC(18,2) DEFAULT 0 NOT NULL,
  importe_haber NUMERIC(18,2) DEFAULT 0 NOT NULL,
  importe_gasto NUMERIC(18,2) DEFAULT 0 NOT NULL,
  importe_recurso NUMERIC(18,2) DEFAULT 0 NOT NULL,
...
CREATE TABLE conta.tplantilla_calculo (
  ...
  codigo_tipo_relacion VARCHAR(20) COLLATE pg_catalog."default",
  prioridad integer NOT NULL, 
  debe_haber varchar(5) NOT NULL, 
  importe NUMERIC NOT NULL, 
  descripcion VARCHAR(200) COLLATE pg_catalog."default", 
  tipo_importe VARCHAR(15) NOT NULL, 
  importe_presupuesto NUMERIC DEFAULT 0 NOT NULL,
  descuento VARCHAR(3) DEFAULT 'no' NOT NULL,
  sw_registro VARCHAR(5) DEFAULT 'si' NOT NULL,
  sw_restar VARCHAR(8) DEFAULT 'no' NOT NULL,
  usar_cc_original VARCHAR(8) DEFAULT 'no'::character varying NOT NULL,
  imputar_excento VARCHAR(8) DEFAULT 'no'::character varying NOT NULL,
  reset_partida_eje VARCHAR(2) DEFAULT 'no' NOT NULL,
...
CREATE TABLE conta.tint_transaccion(
  ...
  banco VARCHAR(3) DEFAULT 'no' NOT NULL,
  ...
  monto_no_ejecutado NUMERIC DEFAULT 0 NOT NULL,
  monto_no_ejecutado_mb NUMERIC DEFAULT 0 NOT NULL,
  id_cuenta_bolsa INTEGER,
  id_partida_ejecucion_tmp INTEGER,
  temp_importe_ma_aux NUMERIC,
  cerrado VARCHAR(10) DEFAULT 'no' NOT NULL,
  fecha_cerrado TIMESTAMP WITHOUT TIME ZONE,
  id_int_comprobante_cierre INTEGER,
  id_origen INTEGER,
  nro_tramite VARCHAR(70),
  forzar_comprometer VARCHAR DEFAULT 'no' NOT NULL,
  id_centro_costo_ori INTEGER,
  nro_tramite_auxiliar VARCHAR(70),
  sw_edit VARCHAR(10) DEFAULT 'no' NOT NULL,
...
CREATE TABLE conta.tint_rel_devengado (
  ...
  sw_reversion VARCHAR(4) DEFAULT 'no' NOT NULL,
  monto_pago_ma NUMERIC,
...
CREATE TABLE conta.ttipo_relacion_comprobante (
  ...
  filtrar_moneda VARCHAR(2) DEFAULT 'si' NOT NULL,
...
CREATE TABLE conta.tresultado_plantilla (
  ...
  codigo VARCHAR(100),
  nombre VARCHAR,
  tipo VARCHAR(15) DEFAULT 'reporte' NOT NULL,
  cbte_aitb VARCHAR(20) DEFAULT 'no' NOT NULL,
  cbte_apertura VARCHAR(20) DEFAULT 'no' NOT NULL,
  cbte_cierre VARCHAR(20) DEFAULT 'no' NOT NULL,
  periodo_calculo VARCHAR(20) DEFAULT 'gestion' NOT NULL,
  glosa VARCHAR,
  id_clase_comprobante INTEGER,
  id_tipo_relacion_comprobante INTEGER,
  relacion_unica VARCHAR(6) DEFAULT 'no' NOT NULL,
  nombre_func VARCHAR,
  visible_menu VARCHAR(2) DEFAULT 'si' NOT NULL,
  visible VARCHAR(50) DEFAULT 'si' NOT NULL,
...
CREATE TABLE conta.tresultado_det_plantilla (
  ...
  montopos INTEGER,
  id_resultado_plantilla INTEGER NOT NULL,
  visible VARCHAR(4) DEFAULT 'si' NOT NULL,
  incluir_cierre VARCHAR(15) DEFAULT 'todos' NOT NULL,
  incluir_apertura VARCHAR(20) DEFAULT 'todos' NOT NULL,
  negrita VARCHAR(4) DEFAULT 'no' NOT NULL,
  cursiva VARCHAR(4) DEFAULT 'no' NOT NULL,
  espacio_previo INTEGER DEFAULT 0 NOT NULL,
  incluir_aitb VARCHAR(20) DEFAULT 'todos' NOT NULL,
  signo_balance VARCHAR(20) DEFAULT 'defecto_cuenta' NOT NULL,
  tipo_saldo VARCHAR(20) DEFAULT 'balance' NOT NULL,
  relacion_contable VARCHAR(50),
  codigo_partida VARCHAR(50),
  id_auxiliar INTEGER,
  destino VARCHAR(20) DEFAULT 'reporte' NOT NULL,
  orden_cbte NUMERIC,
  id_tipo_cc INTEGER,
  salta_hoja VARCHAR(3) DEFAULT 'no' NOT NULL,
  observacion VARCHAR,
...
CREATE TABLE conta.tresultado_dep (
  ...
  prioridad NUMERIC,
  obs VARCHAR,
  id_resultado_plantilla_hijo INTEGER NOT NULL,
...
CREATE TABLE conta.tdoc_compra_venta (
  ...
  id_periodo INTEGER,
  nro_dui VARCHAR(16) DEFAULT 0 NOT NULL,
  id_moneda INTEGER  NOT NULL,
  importe_retgar NUMERIC(18,2) DEFAULT 0 NOT NULL,
  importe_anticipo NUMERIC(18,2) DEFAULT 0 NOT NULL,
  importe_pendiente NUMERIC(18,2) DEFAULT 0 NOT NULL,
  importe_neto NUMERIC(19,2) DEFAULT 0 NOT NULL,
  id_proveedor INTEGER,
  id_cliente INTEGER,
  id_auxiliar INTEGER,
  id_tipo_doc_compra_venta INTEGER,
  estacion VARCHAR(4),
  id_punto_venta INTEGER,
  id_agencia INTEGER,
  nro_tramite VARCHAR,
  sw_pgs VARCHAR(4) DEFAULT 'no' NOT NULL,
  id_funcionario INTEGER,
  codigo_aplicacion VARCHAR(200),
  id_doc_compra_venta_fk BIGINT,
  id_contrato INTEGER,
  fecha_doc_bk DATE,
  num_tra_bk VARCHAR,
  nota_debito_agencia VARCHAR,
  nro_tramite_aux VARCHAR(50),
  consumido VARCHAR(30) DEFAULT 'no'::character varying NOT NULL COLLATE pg_catalog."default",
...
COMMENT ON COLUMN conta.tdoc_compra_venta.consumido
...
CREATE TABLE conta.tagrupador (
  ...
  incluir_rev VARCHAR(4) DEFAULT 'si' NOT NULL,
  id_gestion INTEGER,
...
CREATE TABLE conta.tagrupador_doc (
  id_agrupador_doc SERIAL NOT NULL,
...
--DROP VIEW  IF EXISTS  conta.vint_comprobante;  --mzm
...
CREATE TABLE conta.tconfig_cambiaria (
  ...
  ope_3 VARCHAR DEFAULT '{MB}->{MA}' NOT NULL,
  id_monedas INTEGER [],
...
CREATE TABLE conta.tint_comprobante_bk (
  ...
  fecha_bk TIMESTAMP WITHOUT TIME ZONE DEFAULT now() NOT NULL,
...
CREATE TABLE conta.tint_transaccion_bk (
  ...
  importe_gasto_mt NUMERIC(18,2),
  importe_recurso_mt NUMERIC(18,2),
  id_int_comprobante_bk INTEGER NOT NULL,
...
CREATE TABLE conta.tajuste (
  ...
  tipo VARCHAR(30) DEFAULT 'bancos' NOT NULL,
...
CREATE TABLE conta.tajuste_det (
  ...
  revisado VARCHAR(4) DEFAULT 'no' NOT NULL,
  dif_manual VARCHAR(4) DEFAULT 'no' NOT NULL,
  id_cuenta_bancaria INTEGER,
  id_auxiliar INTEGER,
  id_partida_ingreso INTEGER,
  id_partida_egreso INTEGER,
  id_moneda_ajuste INTEGER,
...
CREATE TABLE conta.ttipo_estado_columna (
  ...
  id_tipo_estado_cuenta INTEGER NOT NULL,
  prioridad NUMERIC DEFAULT 1 NOT NULL,
  descripcion VARCHAR,
  nombre_clase VARCHAR(300),
  parametros_det VARCHAR DEFAULT '{}' NOT NULL,
  clase VARCHAR(300),
