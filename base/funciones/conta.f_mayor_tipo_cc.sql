...
CREATE OR REPLACE FUNCTION conta.f_mayor_tipo_cc (
...
BEGIN
  	 v_nombre_funcion:='conta.f_mayor_tipo_cc';
     ...
     IF  v_registros.movimiento = 'si' THEN
          ...
          inner join pre.tpresupuesto p on p.id_centro_costo = cc.id_centro_costo
          ...
          where 
              ...
              (CASE WHEN p_incluir_adm = 'no' THEN
                 tp.movimiento != 'administrativo'
               ELSE
                  true
               END
              )
     ...
     -- si no,  es una cuenta titular
        -- for, listamos los hijos de la cuenta 
         FOR v_registros in (
               ...
               v_resp_aux = conta.f_mayor_tipo_cc(v_registros.id_tipo_cc, 
               									 p_fecha_ini, 
                                                 p_fecha_fin, 
                                                 p_id_deptos, 
                                                 p_incluir_cierre,
     ...
     raise notice 'f_maryo_tipo_cc %',v_resp_final;    
