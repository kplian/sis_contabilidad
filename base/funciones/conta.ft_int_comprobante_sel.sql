...
BEGIN
	...
	***********************************/
    elseif(p_transaccion='CONTA_INCBTEWF_SEL') then
    	...
    	begin
            ...
            WHERE 
            uof.fecha_asignacion <= now()::date and (uof.fecha_finalizacion is null or uof.fecha_finalizacion >= now()::date) and
            usu.id_usuario=p_id_usuario;
            IF v_codigo='DTE' or v_codigo='GAF' THEN
	...
	***********************************/
		...
		begin
            ...
            JOIN orga.tfuncionario fun on fun.id_funcionario=uof.id_funcionario
            JOIN segu.tusuario usu on usu.id_persona=fun.id_persona
            where uof.fecha_asignacion <= now()::date and (uof.fecha_finalizacion is null or uof.fecha_finalizacion >= now()::date) and
            usu.id_usuario=p_id_usuario;
