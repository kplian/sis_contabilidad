...
CREATE OR REPLACE FUNCTION conta.f_mayor_cuenta_tipo_cc (
  p_id_cuenta integer,
  p_fecha_ini date,
  p_fecha_fin date,
  p_id_deptos varchar,
  ...
  p_id_tipo_cc integer = NULL::integer,
  pa_id_centro_costo integer [] = NULL::integer[]
)
RETURNS numeric [] AS
$body$
/*
Autor: RAC, CAP
Fecha: 11/04/2018
Descripcion:   funcion para el maryo combinado de cuenta y tipo de centro de costos

EJEMPO
v_resp_aux  					numeric[];
v_resp_aux = conta.f_mayor_cuenta_tipo_cc(v_registros.id_cuenta, 
               									 p_fecha_ini, 
                                                 p_fecha_fin, 
                                                 p_id_deptos, 
                                                 p_incluir_cierre,
...
BEGIN
	 v_nombre_funcion:='conta.f_mayor_cuenta_tipo_cc';
     va_id_centro_costo = null;
     
    if p_id_tipo_cc is not null then
      WITH RECURSIVE tree AS (
        ...
        select tcc.id_tipo_cc,
                ...
                tcc.tipo,
                tcc.movimiento,
                tcc.id_tipo_cc :: TEXT AS ancestors,
                1                    AS nivel
              ...
              select
                ...
                tcc.tipo,
                tcc.movimiento,
                tree.ancestors || '->' || tcc.id_tipo_cc :: TEXT AS ancestors,
                tree.nivel + 1                    AS nivel
              FROM param.ttipo_cc tcc, tree
              WHERE tcc.id_tipo_cc_fk= tree.id_tipo_cc
        )
        select array_agg(cc.id_centro_costo)
        into va_id_centro_costo
        from tree t1
        join param.tcentro_costo cc on cc.id_tipo_cc=t1.id_tipo_cc
        join param.tgestion g on g.id_gestion=cc.id_gestion
        where t1.movimiento='si'
        and g.fecha_ini<=p_fecha_ini and p_fecha_ini<=g.fecha_fin;
        
        p_id_tipo_cc = NULL;
        pa_id_centro_costo = va_id_centro_costo;
     ...
     IF  v_registros.sw_transaccional = 'movimiento' THEN
          ...
          where 
              ...
              (CASE WHEN p_id_ot is NULL  THEN  
                    ...
                    END) AND 
              (CASE WHEN pa_id_centro_costo is NULL  THEN  
                         0=0 
                    ELSE
                       t.id_centro_costo = ANY (pa_id_centro_costo)
                    END);
     ...
     -- si no es una cuenta titular
        -- for, listamos los hijos de la cuenta 
         FOR v_registros in (select 
               ...
               v_resp_aux = conta.f_mayor_cuenta_tipo_cc(
